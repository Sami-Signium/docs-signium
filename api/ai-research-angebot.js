export default async function handler(req, res) {
  ...
  // ── MODE: EXTRACT from uploaded document ──────────────────────────────────
  if (mode === 'extract') {
  ...
  // ── MODE: COMPANY research via web search ─────────────────────────────────
  if (mode === 'company') {
    const { company } = body;

    const system = isDE
      ? `Du bist ein Executive Search Berater bei Signium Austria. Schreibe ein professionelles Unternehmensprofil für ein Angebot. Stil: sachlich, präzise, 3-5 Absätze, ca. 200-300 Wörter. Inhalt: Gründung/Geschichte, Kerngeschäft, Größe, Marktposition, Eigentümerstruktur. Nur Fließtext, keine Aufzählungen, keine Überschriften. Auf Deutsch.`
      : `You are an Executive Search consultant at Signium Austria. Write a professional company profile for a proposal. Style: factual, precise, 3-5 paragraphs, approx. 200-300 words. Content: founding/history, core business, size, market position, ownership. Plain prose only, no bullets, no headings. In English.`;
  ...
  // ── MODE: POSITION refine from keywords ───────────────────────────────────
  if (mode === 'position') {
    const { company, positionTitle, keywords } = body;

    const system = isDE
      ? `Du bist ein Executive Search Berater bei Signium Austria. Schreibe eine professionelle Positionsbeschreibung für ein Angebot. Stil: präzise, 2-3 Absätze, ca. 150-250 Wörter. Inhalt: Kernaufgaben, Verantwortungsbereich, strategische Bedeutung, Reporting-Linie. Nur Fließtext, keine Aufzählungen. Auf Deutsch.`
      : `You are an Executive Search consultant at Signium Austria. Write a professional position description for a proposal. Style: precise, 2-3 paragraphs, approx. 150-250 words. Content: core tasks, area of responsibility, strategic significance, reporting line. Plain prose only, no bullets. In English.`;
    ...
    try {
      ...
      console.error('Position refine error:', err);
      return res.status(500).json({ error: err.message });
    }
  }
